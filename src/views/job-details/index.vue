<template>
  <div
    class="app-container details-container bg-dark"
    @click="notePopover = false"
  >
    ...
    <div class="flex flex-row space-between app-content">
      <section class="section-wrapper prop-section">
        <div
          ...
          class="section-view job-summary section-summary"
        >
          ...
          <ul class="summary-items">
            ...
            <li style="position: relative">
              ...
              <div class="prop-content notes-content">
                <p
                  id="notesP"
                  :class="{
                    'notes-content-fold': foldPForNote,
                    'notes-content-unfold': !foldPForNote,
                  }"
                >
                  {{ jobInfo.notes }}
                </p>
              ...
              <span
                v-if="noteHint"
                class="notes-can-fold"
                @click="foldForNotes"
              >{{ foldButtonForNote ? "unfold" : "fold" }}</span
              >
            </li>
            <li>
              <hr class="hr-style">
            ...
            <li class="inline-row" style="margin-bottom: 0px">
              ...
              <div class="flex flex-col flex-start prop-content prop-dataset">
                <overflow-tooltip
                  v-for="(item, index) in showingRoleList"
                  :key="index"
                  :content="item"
                  placement="right"
                >
                ...
                >
                  <div class="flex flex-col flex-start">
                    <span
                      v-for="(itemRole, indexRole) in thisRoleList"
                      :key="indexRole"
            ...
            <li
              v-for="(item, index) in otherRoleList"
              :key="index"
              class="inline-row"
            >
              <div class="prop inline-prop">
              ...
              <div class="flex flex-center prop-content">
                ...
                >
                  <div>
                    ...
                    <el-row
                      v-for="(dataset, index) in item.datasetList"
                      :key="index"
                    >
                      <el-col :span="8">
                        <p>{{ dataset.name }}</p>
                      ...
                      <el-col :offset="4" :span="12">
                        <div class="flex flex-col">
                          <p
                            v-for="(item, index) in checkDataSetForOther(
                              dataset.dataset
                            )"
                            :key="index"
                          >
            ...
            <li
              class="inline-row download-link"
            ...
            <li
              :class="{ 'disable-color': role.toLowerCase() === 'arbiter' }"
              class="inline-row download-link"
      ...
      <section class="section-wrapper echart-section">
        <div class="output-wrapper flex flex-col">
          ...
          <div class="flex flex-row output-content">
            ...
            <div
              v-loading="paraLoading"
              class="para-wrapper flex flex-col space-between"
            >
              <div
                class="flex flex-col flex-start para-warpper-content"
                style="width: 100%"
              >
                ...
                >
                  <el-tree
                    v-if="treeRefresh"
                    ref="foldParameterTree"
                    :data="paramList"
                    :default-expand-all="treeUnfoldAll"
                    :empty-text="''"
                    :props="defaultPropsForTree"
                    class="bg-dark"
                  />
                  <div
                    v-if="paramList && paramList.length > 0"
                    class="unfold-tree"
                    @click.stop="unfoldAll"
                  >
                    {{ treeUnfoldAll ? "fold all" : "unfold all" }}
                  </div>
                </div>
              </div>
              <el-button
...
import { getComponentCommand, getComponentPara, jobDownload, retryJob } from '@/api/job'
  ...
  methods: {
    ...
    shouldShowPopover(item, id) {
      ...
      for (let i = 0; i < this.showingRoleList.length; i++) {
        const width = this.measureText(ctx, this.showingRoleList[i] || '', {
          font: 12 * 1.14 + 'px Arial'
        }).width
        const acWidth = parseInt(
          getComputedStyle(
            document.getElementById('spanPopOver' + i)
          ).width.replace('px', '')
        )
    ...
    notesHint() {
      ...
      const width = this.measureText(cvs, this.jobInfo.notes || '', {
        size: 14,
        weight: 'bold'
      }).width
      const acWidth = parseInt(
        getComputedStyle(document.getElementById('notesP')).width.replace(
          'px',
          ''
        )
      )
    ...
    visualization() {
      if (
        ...
        this.lastComponentName === this.componentName
      ) {
        this.$refs.outputDialog.refresh()
      }
    ...
    sectionMove() {
      if (this.scrollHoldChange) {
        this.scrollTopPos = document.getElementById(
          'sectionWrapperScroll'
        ).scrollTop
    ...
    setData(data) {
      const {
        summary_date: { job, dataset: _dataset },
        dependency_data
      } = data
      if (_dataset) {
    ...
    transformJobInfo(job) {
      return {
        submmissionTime: job.fCreateTime
          ? parseTime(new Date(job.fCreateTime))
          : '',
    ...
    jobRetry(name) {
      const vm = this
      const confirmText = [
        `The job will continue from where it ${this.jobInfo.status}`,
        'it may take few seconds to  update job status.'
      ]
      this.$refs.confirm.confirm(...confirmText).then(() => {
        vm.restartJobWebsocket(name)
      })
    },
    downloadFile(res) {
      this.$refs.downloadReport.downloadFiles(
        res,
        this.jobId + '_' + this.componentName
      )
    },
