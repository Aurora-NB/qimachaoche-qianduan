<template>
  <div
    class="app-container flex flex-col history-container bg-dark"
    @click="uploadEditor($event)"
  >
    ...
    <div
      ...
      class="flex flex-col space-between table-wrapper"
    >
      <el-table
        ref="currentRowTable"
        :data="list"
...
import { deepClone, formatSeconds, parseTime } from '@/utils'
import { mapGetters } from 'vuex'
import { addNotes, killJob, queryFileds, queryJobs, retryJob } from '@/api/job'
...
export default {
  ...
  methods: {
    ...
    queryList(filterOperation) {
      ...
      const req = () => {
        return queryJobs(para).then((res) => {
          this.saveCondition = deepClone(this.condition)
          ...
          res.data.list.forEach((item) => {
            ...
            if (job) {
              ...
              // })
              partner = Array.isArray(job.partners)
                ? job.partners.join(',')
                : '-'
            }
            // if (dataset) {
    ...
    onRetry(row) {
      ...
      const confirmText = isDone
        ? [
          `The job will continue from where it ${status}`,
      ...
      this.$refs.confirm.confirm(...confirmText).then(() => {
        ...
        callback(mid).then(() => {
          this.queryList((job) => {
            return (
              job.fJobId !== row.jobId ||
              (isDone
                ? job.fStatus
                  .toLowerCase()
                  .match(new RegExp(`(${['waiting', 'running'].join('|')})`))
                : isDoing
                  ? job.fStatus
                    .toLowerCase()
                    .match(
                      new RegExp(
                        `(${['failed', 'canceled', 'success'].join('|')})`
                      )
                    )
                  : false)
            )
