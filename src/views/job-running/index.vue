<template>
  <div
    v-loading="loading"
    class="running-container flex flex-center flex-col app-container"
  >
    <ul class="job-list flex flex-center flex-wrap ie-ul">
      <li v-for="(item, index) in jobList" :key="index" class="shadow">
        <job-running
          ...
          @kill="
            handleKillJob(item.jobId, item.role, item.partyId, 'cancel', index)
          "
        />
      </li>
    </ul>
      ...
      <div class="flex justify-center" style="margin-top: 72px">
        <button
          :class="[
            ...
            checkclick === true ? 'dialog-click-button' : '',
          ]"
          class="dialog-button"
          @click="sureKillJob"
        ...
        <button
          :class="[
            checkSure === false
              ? 'dialog-check-button'
              : 'dialog-uncheck-button',
            checkclick === false ? 'dialog-click-button' : '',
          ]"
          class="dialog-button"
