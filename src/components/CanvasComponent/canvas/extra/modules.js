...
const progressComp = {
  ...
  animations: {
    ...
    loading() {
      const lay = this
      const changeList = []
      changeList.push(lay.settingRGBA(lay.contentStyle, 'rgba(36,182,139,0)', (to) => {
        lay.contentStyle = to
      }, 500))
      ...
      changeList.push(lay.settingNum(0, 1, (to) => {
        lay.progress = to
      }, 3000, 20))
    ...
    toNewType(type, timeOrPic) {
      ...
      if (type !== progressComp.type.RUNNING) {
        ...
        changeList.toNewTypeContent.push(lay.settingNum(lay.progress || 1, 1, (to) => {
          lay.progress = to
        }, 200))
      }
      const toColor = _getContainerColor(type, !!lay.choose, lay.disable, 'content')
      changeList.toNewTypeContent.push(lay.settingRGBA(lay.contentStyle, toColor, (to) => {
        lay.contentStyle = to
      }, 200))
      ...
      changeList.toNewTypeContainer.push(lay.settingRGBA(lay.containerStyle, toContainerColor, (to) => {
        lay.containerStyle = to
      }, 200))
      const toFontStyle = _getTextColor(type, !!lay.choose, lay.disable)
      changeList.toNewTypeText.push(lay.settingRGBA(lay.fontStyle, toFontStyle, (to) => {
        lay.fontStyle = to
      }, 200))
