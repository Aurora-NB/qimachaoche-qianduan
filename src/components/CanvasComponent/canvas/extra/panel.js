...
const panelComp = {
  ...
  events: {
    progress(progress) {
      const lay = this
      lay.registerChainTranslate('progress', false, lay.settingNum(lay.progress, progress, (to) => {
        lay.progress = to
      }, 200))
    },
    setTime(time) {
...
function drawContainer(ctx, start, radius, width, gradient = true) {
  if (gradient) {
    ctx.strokeStyle = createGradient(ctx, { x: start.x, y: start.y }, { x: start.x + width, y: start.y }, [{
      pos: 0,
      color: 'rgba(73, 78, 206, 0.6)'
    }, { pos: 1, color: '#24b68b' }])
    ctx.lineWidth = 10
    ctx.lineCap = 'butt'
  } else {
...
const path = function() {
  ...
  }, lay.r - distance, lay.width - Math.sin(Math.PI / 4) * distance * 2, false)
